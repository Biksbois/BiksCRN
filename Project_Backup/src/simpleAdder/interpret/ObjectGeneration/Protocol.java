...
public class Protocol extends HelpMethods{
    ...
    public String ApplyMix(protocolOperation protocol){
        ...
        prettyResult += HelpGenerateEuler(global.get(protocol.mix.ResultingSample).scope,level,Integer.toString(mixCount));
        ...
        if (global.containsKey(vv.SPECIE)){
            for (String key: global.get(vv.SPECIE).species.keySet()){
                if (VerifySpecie(key, local)){
                    PrettyResult += PM.ApplyTap(level, "sample[\""+ key +"\"] = [" + GetSampleName(protocol.equili.sample) + ".sample.get(\"" + key + "\")[-1]]\n");
                }
            }
        }
